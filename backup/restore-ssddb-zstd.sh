...
if [ -z "$1" ]; then
  echo "ERROR:  Define SQL.ZST dump-file as Argument #1 !  Example:"
  echo "$0  ssddb_2024-03-05_150116.sql.zst"
  exit 0
fi
...
zstd -d -c $DUMP_FNAME | psql -h $PG_HOST -p $PG_PORT -d $PG_DB -U $PG_USER
